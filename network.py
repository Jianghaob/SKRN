...
class res2_SK_GAatt(nn.Module):
    ...
    def __init__(self, band, classes):
        ...
        self.conv6 = nn.Conv3d(in_channels=1, out_channels=24, padding=(1, 1, 0),       #从这里开始进行空间上的卷积
        ...
        self.global_pooling = nn.AdaptiveAvgPool3d(1) #对7*7*1，60的特征图进行pooling操作，得到  1*1*1,60
        self.full_connection = nn.Sequential(
            nn.Dropout(p=0.5),
            nn.Linear(60, classes) #再对60个进行drop和线性
            # nn.Softmax()
        )
    ...
    def forward(self, X):
        ...
        # 对 （bitchsize，num，W,H,C）重组 即（32,200,9,9,1）  使其变为（32,1,9,9,200）
        x6 = x6.permute(0, 4, 2, 3, 1)
        # print(x6.shape)

        x7 = self.batch_norm5(x6)
        x7 = self.conv6(x7)  # （32,24,3,3,1）
        ...
        x10 = x10.view(x10.size(0), -1)  # （32,60）
