...
def index_assignment(index, row, col, pad_length):#传入索引，位置，返回选中的数据的对角位置，返回字典
    new_assign = {}
...
def select_small_cubic(data_size, data_indices, whole_data, patch_length, padded_data, dimension):
    small_cubic_data = np.zeros((data_size, 2 * patch_length + 1, 2 * patch_length + 1, dimension)) #假设(100,28,28,3)
